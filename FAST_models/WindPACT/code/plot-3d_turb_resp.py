...
#
## turbulence axes
#ax_t = plt.axes([0.07,0.5,0.13,0.2])
#
## front-facing turbine
#ax_f = plt.axes([0.3,0.27,0.3,0.7])
#
## left-facing turbine
#ax_l = plt.axes([0.65,0.27,0.3,0.7])
#
## blade response
#ax_b = plt.axes([0.07,0.05,0.88,0.15])
...
max_defl = -min(OoPDefl1.min(),OoPDefl2.min(),OoPDefl3.min())
min_defl = -max(OoPDefl1.max(),OoPDefl2.max(),OoPDefl3.max())
...
for i_t in range(Azimuth.size):
    ...
    # =============== blade deflections ===============
    ax_b.plot([t_plot[i_t],t_plot[i_t]],[-max_defl,-min_defl],'k:')
    ax_b.plot(t_plot,OoPDefl)
    ax_b.set_ylim([-max_defl,-min_defl])
