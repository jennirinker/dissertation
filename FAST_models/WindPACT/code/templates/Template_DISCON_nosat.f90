...
!DEC$ ATTRIBUTES DLLEXPORT, ALIAS:'DISCON' :: DISCON
   ...
   ! generator-torque controller and PID collective blade pitch controller
   ! to match the WindPACT controller. This routine was written by 
   ! J. Rinker of Duke University/NREL/NWTC.
...
REAL(4)                      :: DerSpdErr                                       ! Current derivative of speed error w.r.t. time, rad/s.
REAL(4)                      :: ElapTime                                        ! Elapsed time since the last call to the controller, sec.
REAL(4), PARAMETER           :: CornerFreq    =       {:8.6f}                  ! Corner frequency (-3dB point) in the recursive, single-pole, low-pass filter, rad/s. -- chosen to be 1/4 the blade edgewise natural frequency ( 1/4 of approx. 1Hz = 0.25Hz = 1.570796rad/s)
...
REAL(4)                      :: LastGenSpeedF                                   ! Last filtered generator speed, rad/s.
...
REAL(4), PARAMETER           :: PC_DT         = {:7.5f}                         ! Communication interval for pitch  controller, sec.
REAL(4), PARAMETER           :: PC_GSPit1     = {:7.5f}                         ! Starting angle for gain scheduling [rad]
REAL(4), PARAMETER           :: PC_GSPit2     = {:7.5f}                         ! Ending angle for gain scheduling [rad]
REAL(4), PARAMETER           :: PC_GSCoef     = {:7.5f}                         ! Gain scheduling coefficient
REAL(4), PARAMETER           :: PC_GSExp      = {:7.5f}                        ! Gain scheduling exponent
REAL(4), PARAMETER           :: PC_KD         =       {:11.9f}               ! Derivative gain for pitch controller at rated pitch angle, (-).
REAL(4), PARAMETER           :: PC_KI         =       {:11.9f}               ! Integral gain for pitch controller at rated pitch angle, (-).
REAL(4), PARAMETER           :: PC_KP         =       {:11.9f}               ! Proportional gain for pitch controller at rated pitch (zero), sec.
REAL(4), PARAMETER           :: PC_MaxPit     =       {:9.7f}                 ! Maximum pitch setting in pitch controller, rad.
REAL(4), PARAMETER           :: PC_MaxRat     =       {:9.7f}                 ! Maximum pitch  rate (in absolute value) in pitch  controller, rad/s.
REAL(4), PARAMETER           :: PC_MinPit     =       {:9.7f}                 ! Minimum pitch setting in pitch controller, rad.
REAL(4), PARAMETER           :: PC_RefSpd     =     {:8.4f}                    ! Desired (reference) HSS speed for pitch controller, rad/s.
...
REAL(4), PARAMETER           :: VS_CtInSp     =      {:8.5f}                   ! Transitional generator speed (HSS side) between regions 1 and 1 1/2, rad/s.
REAL(4), PARAMETER           :: VS_DT         =       {:7.5f}       !JASON:THIS CHANGED FOR ITI BARGE: 0.0001 ! Communication interval for torque controller, sec.
REAL(4), PARAMETER           :: VS_MaxRat     =   {:7.1f}                       ! Maximum torque rate (in absolute value) in torque controller, N-m/s.
REAL(4), PARAMETER           :: VS_MaxTq      =   {:8.2f}                      ! Maximum generator torque in Region 3 (HSS side), N-m. -- chosen to be 10% above VS_RtTq = 43.09355kNm
REAL(4), PARAMETER           :: VS_Rgn2K      =       {:8.6f}                  ! Generator torque constant in Region 2 (HSS side), N-m/(rad/s)^2.
REAL(4), PARAMETER           :: VS_Rgn2Sp     =      {:8.5f}                   ! Transitional generator speed (HSS side) between regions 1 1/2 and 2, rad/s.
REAL(4), PARAMETER           :: VS_Rgn3MP     =       {:10.8f}                ! Minimum pitch angle at which the torque is computed as if we are in region 3 regardless of the generator speed, rad. -- chosen to be 1.0 degree above PC_MinPit
REAL(4), PARAMETER           :: VS_RtGnSp     =     {:8.4f}                    ! Rated generator speed (HSS side), rad/s. -- chosen to be 99% of PC_RefSpd
REAL(4), PARAMETER           :: VS_RtPwr      = {:9.1f}                       ! Rated generator generator power in Region 3, Watts. -- chosen to be 5MW divided by the electrical generator efficiency of 94.4%
...
REAL(4), PARAMETER           :: VS_SlPc       =      {:4.1f}                       ! Rated generator slip percentage in Region 2 1/2, %.
...
!   GK         = 1.0/( 1.0 + PitCom(1)/PC_KK )   ! This will ensure that the pitch angle is unchanged if the initial SpdErr is zero
   IF PitCom(1) <= PC_GSPit1 THEN 		! Gain scheduling from WindPACT study (Malcolm, 2006)
      GK = 1.0
   ELSEIF PitCom(1) >= PC_GSPit2 THEN
      GK = PC_GSCoef * (PC_GSPit2**PC_GSExp)
   ELSE
      GK = PC_GSCoef * (PitCom(1)**PC_GSExp)
   ENDIF
...
ENDIF ! End first initialization
...
!=======================================================================
   ...
   GenSpeedF     = ( 1.0 - Alpha )*GenSpeed + Alpha*LastGenSpeedF
...
!         GenTrq = VS_RtPwr/GenSpeedF
      IF ( (   GenSpeedF >= VS_RtGnSp )  THEN 					 ! We are in region 3 - power is constant
         GenTrq = VS_RtPwr/VS_RtGenSpeed
...
!      GK = 1.0/( 1.0 + PitCom(1)/PC_KK )
   ...
   !   integral term using the pitch angle limits:
      ...
      DerSpdErr = (GenSpeedF - LastGenSpeedF)/ElapTime
   ...
   !   gains:
      ...
      PitComD   = GK*PC_KD*DerSpdErr                                    ! Derivative term


   ! Superimpose the individual commands to get the total pitch command;
   !   saturate the overall command using the pitch angle limits:

      PitComT   = PitComP + PitComI + PitComD                           ! Overall command (unsaturated)
