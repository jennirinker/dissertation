...
!=======================================================================
SUBROUTINE WrappedCauchy( Rho, Mu, WrapCauchNum )

! This subroutine produces random numbers from a wrapped Cauchy
! distribution based on the pRNG that is requested in TurbSim's input file. 
! This routine assumes that the random number generator has been initialized 
! earlier in the main program.
!
...
REAL(ReKi), ALLOCATABLE          :: BoolArray(:)                ! Array of +/- 1 values
REAL(ReKi)                       :: c                           ! parameter in wrapped Cauchy sample method
REAL(ReKi), INTENT(IN)           :: Mu                          ! location parameter
INTEGER                          :: N                           ! sample size
REAL(ReKi)(16), PARAMETER        :: Pi = 4 * ATAN(1.0_16)       ! Pi
REAL(ReKi), INTENT(IN)           :: Rho                         ! concentration parameter
REAL(ReKi), ALLOCATABLE          :: U(:)                        ! uniform random variables to transform
REAL(ReKi), ALLOCATABLE          :: V(:)                        ! parameter in wrapped Cauchy sample method
REAL(ReKi), INTENT(INOUT)        :: WCSample(:)                 ! sample of Wrapped Cauchy random variables
...
ALLOCATE( U(N) )
   ...
   ! Create Boolean array of +/- ones

CALL RndUnif(BoolArray)
