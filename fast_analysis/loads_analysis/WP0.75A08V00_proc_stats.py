...
fast_dict = mainFAST.ReadFASTFile(files[0])
...
for i_f in range(n_files):
    ...
    # load FAST file
    fast_dict   = mainFAST.ReadFASTFile(fname)
    ...
    if set(fields).symmetric_difference(fast_fields):
        ...
        for i_stat in range(len(calc_stats)):
            if ( calc_stats[i_stat] == 'min' ):
                ...
                err_str = 'Statistic {:s}'.format(calc_stat[i_stat]) + \
                    ' is not coded.'
                ValueError(err_str)
           
# create dictionary and save in script directory
os.chdir(orig_dir)
out_fname = turb_name + '_stats.mat'
out_dict = {}
