...
!BONNIE: add the UsrVKM model to this list when the model is complete as well as USRINP
         CALL TS_Abort ( 'The turbulence model must be "IECKAI", "IEC_TC", "IECVKM", "SMOOTH",' &
      ...
      SELECT CASE ( IECstandard )
         ...
         CASE ( 3 )
            ...
            IF (IECedition < 1 ) THEN  ! Set up the default
               ...
               IF ( ( TurbModel /= 'IECKAI' ) .AND. ( TurbModel /= 'IEC_TC' ) ) THEN
...
!!$! -- end block --
   ...
   ELSE  !not IECKAI and not IEC_TC and not IECVKM and not MODVKM
...
SELECT CASE ( TRIM(TurbModel) )
   CASE ( 'GP_LLJ' )
      WindProfileType = 'JET'
   CASE ( 'IECKAI','IEC_TC','IECVKM','IECMAN','MODVKM' )
      WindProfileType = 'IEC'
   CASE ( 'TIDAL' )
      WindProfileType = 'H2L'
   CASE ( 'USRVKM' )
...
IF ( TurbModel /= 'IECKAI' .AND. TurbModel /= 'IEC_TC' .AND. TurbModel /= 'IECVKM' ) THEN
...
ENDIF    ! Not IECKAI and Not IECVKM and not IEC_TC

IF ( TurbModel /= 'IECKAI' .AND. TurbModel /= 'IEC_TC' .AND. TurbModel /= 'IECVKM' .AND. TurbModel /= 'MODVKM' .AND. TurbModel /= 'IECMAN') THEN
...
ELSE  ! IECVKM or IECKAI or IEC_TC models
