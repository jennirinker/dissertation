...
#for i_instr in [1]:
for i_instr in range(2):
    
    fignum = fignum + i_instr

    fieldname_dir = [s for s in fields if ('Direction' in s)][i_instr]
    fieldname_spd = [s for s in fields if ('Wind_Speed_' in s)][i_instr]
    ...
    # screen too-large wind speeds for Texas Tech data
    if dataset == 'texastech':
        clean = clean[clean[:,fields.index('Wind_Speed_Sonic')] < 30.,:]
    ...
    if i_instr == 0:
        wind_dirs = (180. + 75. - wind_dirs) % 360
    elif i_instr == 1:
        wind_dirs = (180. + 30. - wind_dirs) % 360
    ...
    for i_id in range(n_instr):
        ...
        if (dataset in ('NREL','fluela')):
            ...
            if i_instr == 0:
                ax_title = 'UVW: {:d} m'.format(ID)
            elif i_instr == 1:
                ax_title = 'Sonic: {:d} m'.format(ID)
