...
for dataset in datasets:
    ...
    # custom figuze sizes, labels, etc. for each dataset
    fieldname_ID  = 'ID'
    if (dataset == 'NREL'):
        fignum = 1
        figsize = (6.5,4.5)
        nax_y,nax_x = 2, 3
        perc_max = 0.15
        fieldname_spd = 'Mean_Wind_Speed'
        fieldname_dir = 'Wind_Direction'
    elif (dataset == 'fluela'):
        fignum = 2
        figsize = (6.5,2.5)
        nax_y,nax_x = 1,3
        ...
        perc_max = 0.14
        fieldname_spd = 'Mean_Wind_Speed'
        fieldname_dir = 'Wind_Direction_Sonic'
    else:
        KeyError('Dataset not coded')
        
    print(fieldname_ID,fieldname_dir,fieldname_spd)
    ...
    ID_data   = screen[:,fields.index(fieldname_ID)]
    wind_spds = screen[:,fields.index(fieldname_spd)]
    wind_dirs = screen[:,fields.index(fieldname_dir)] % 360
    ...
    #    print(bottom.max(),sum(bottom))
        ...
        if (dataset in ('NREL','fluela','texastech')):
