...
!bjj: I have no idea why it is necessary to be a factor of 4, so I'm removing it for now:      ! Make sure it is a multiple of 2 too.
   ...
   IF ( p_grid%Periodic ) THEN
      ...
      IF (p_grid%NoPad) THEN
         NumSteps2          = ( p_grid%NumSteps - 1 )/2 + 1
         p_grid%NumSteps    = 2*NumSteps2
...
SUBROUTINE CalcTempCoh(p, Rho, Mu, ErrStat, ErrMsg)
   ...
   TYPE(TurbSim_ParameterType),  INTENT(in)     :: p                            !< TurbSim parameters
   REAL(ReKi),                   INTENT(  OUT)  :: Rho           (:,:)          !< The array of concentration parameters (NPoints,3)
   REAL(ReKi),                   INTENT(  OUT)  :: Mu            (:,:)          !< The array of location parameters (NPoints,3)
...
SUBROUTINE SetPhaseAngles( p, OtherSt_RandNum, Rho, Mu, PhaseAngles, ErrStat, ErrMsg )
   ...
   REAL(ReKi),                   INTENT(IN   ) :: Rho           (:,:)                            !< temporal coherence concentration parameters
   REAL(ReKi),                   INTENT(IN   ) :: Mu            (:,:)                            !< temporal coherence location parameters
