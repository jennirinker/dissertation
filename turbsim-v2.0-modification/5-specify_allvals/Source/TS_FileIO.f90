...
SUBROUTINE ReadInputFile(InFile, p, OtherSt_RandNum, ErrStat, ErrMsg)
  ...
  ! This subroutine is used to read parameters from the input file.
   ...
   REAL(ReKi)                    :: InTCVar    (2)                   ! Contains the temporal coherence parameters (used for input)
...
!..................................................................................................................................     
   ...
   CALL ReadCom( UI, InFile, "Temporal Coherence Models Heading Line 2", ErrStat2, ErrMsg2, UnEc )
      ...
      ! ------------ Read in the u component temporal coherence parameters, Rho(1) and Mu(1) ------------
   CALL ReadRAryDefault( UI, InFile, InTCVar, "TCMod1", "u-component temporal coherence parameters", UnEc, UseDefault, ErrStat2, ErrMsg2 )
      CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'ReadInputFile')
      IF ( .NOT. UseDefault ) THEN         
         p%met%Rho(1) = InTCVar(1)
         p%met%Mu(1)  = InTCVar(2)
      END IF

      ! ------------ Read in the v component temporal coherence parameters, Rho(2) and Mu(2) ------------
   CALL ReadRAryDefault( UI, InFile, InTCVar, "TCMod2", "v-component temporal coherence parameters", UnEc, UseDefault, ErrStat2, ErrMsg2 )
      CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'ReadInputFile')
      IF ( .NOT. UseDefault ) THEN         
         p%met%Rho(2) = InTCVar(1)
         p%met%Mu(2)  = InTCVar(2)
      END IF

      ! ------------ Read in the w component temporal coherence parameters, Rho(3) and Mu(3) ------------
   CALL ReadRAryDefault( UI, InFile, InTCVar, "TCMod3", "w-component temporal coherence parameters", UnEc, UseDefault, ErrStat2, ErrMsg2 )
      CALL SetErrStat( ErrStat2, ErrMsg2, ErrStat, ErrMsg, 'ReadInputFile')
      IF ( .NOT. UseDefault ) THEN         
         p%met%Rho(3) = InTCVar(1)
         p%met%Mu(3)  = InTCVar(2)
      END IF

!..................................................................................................................................
!  Do some error checking on the temporal coherence values
!..................................................................................................................................         
   ...
   IF (p%met%Rho(1) < 0.0 .OR. p%met%Rho(1) > 1.0) THEN
      CALL SetErrStat( ErrID_Fatal, 'The value of Rho1 must be between 0 and 1.', ErrStat, ErrMsg, 'ReadInputFile')
   ENDIF

   IF (p%met%Rho(2) < 0.0 .OR. p%met%Rho(2) > 1.0) THEN
      CALL SetErrStat( ErrID_Fatal, 'The value of Rho2 must be between 0 and 1.', ErrStat, ErrMsg, 'ReadInputFile')
   ENDIF

   IF (p%met%Rho(3) < 0.0 .OR. p%met%Rho(3) > 1.0) THEN
      CALL SetErrStat( ErrID_Fatal, 'The value of Rho3 must be between 0 and 1.', ErrStat, ErrMsg, 'ReadInputFile')
   ENDIF
